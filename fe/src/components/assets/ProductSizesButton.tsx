...
const ProductSizesButton = ({
  ...
  const styleSize = (size: number | undefined) => {
    ...
    if (size === 0)
      return "bg-gray-100 text-gray-400 border-gray-400 cursor-not-allowed";
    return null;
  };
      ...
      <div className="flex gap-1">
        {sizesData.map((size) => (
          <button
            ...
            className={`w-8 h-8 rounded-full border border-black text-xs ${
              chooseSize == size ? "bg-black text-white" : ""
            } ${styleSize(sizes?.[size as keyof typeof sizes])}`}
          >
...
export default ProductSizesButton;
